...
export function interpolate(
  ...
  return visit(ast, {
    ...
    visitTemplateLiteral(path) {
      const canTransformToStringLiteral = (
        path.node.expressions as any[]
      ).every(
        (expression) =>
  ...
  visit(node, {
    visitIdentifier(path) {
      if (nameToIdentifier.hasOwnProperty(path.node.name)) {
        contained.push(path.node as namedTypes.Identifier);
      }
      this.traverse(path);
    },
    // Recast has a bug of traversing class decorators
  ...
  visit(node, {
    visitClassDeclaration(path) {
      if (path.node.id && path.node.id.name === id.name) {
        classDeclaration = path.node as namedTypes.ClassDeclaration;
        return false;
      }
      return this.traverse(path);
    },
...
export function addIdentifierToConstructorSuperCall(
  ...
  visit(ast, {
    visitClassMethod(path) {
      ...
      if (isConstructor(classMethodNode as namedTypes.ClassMethod)) {
